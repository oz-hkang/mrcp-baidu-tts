...
static apt_bool_t xfyun_synth_engine_destroy(mrcp_engine_t *engine)
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] destroy synthesizer engine");
...
static apt_bool_t xfyun_synth_engine_open(mrcp_engine_t *engine)
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] open synthesizer engine");
	xfyun_synth_engine_t *xfyun_engine = engine->obj;
	if(xfyun_engine->task) {
		apt_task_t *task = apt_consumer_task_base_get(xfyun_engine->task);
		apt_task_start(task);
...
static apt_bool_t xfyun_synth_engine_close(mrcp_engine_t *engine)
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] close synthesizer engine");
	xfyun_synth_engine_t *xfyun_engine = engine->obj;
	if(xfyun_engine->task) {
		apt_task_t *task = apt_consumer_task_base_get(xfyun_engine->task);
...
{
	/* nothing to destroy */
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] Destroy engine synthesizer channel");
	return TRUE;
}
...
static apt_bool_t xfyun_synth_channel_open(mrcp_engine_channel_t *channel)
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] Open engine synthesizer channel");
...
static apt_bool_t xfyun_synth_channel_close(mrcp_engine_channel_t *channel)
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] Close engine synthesizer channel");
...
static apt_bool_t xfyun_synth_channel_request_process(mrcp_engine_channel_t *channel, mrcp_message_t *request)
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] xfyun_synth_channel_request_process");
...
static apt_bool_t xfyun_synth_text_to_speech(const char* src_text, const char* params, mpf_buffer_t *buffer) {
	...
	apt_log(APT_LOG_MARK, APT_PRIO_WARNING,"[xfyun]  In the synthesis process ...");
	while (1) 
	{
		/* get the audio data */
	...
	/* completed */
    apt_log(APT_LOG_MARK, APT_PRIO_WARNING,"[xfyun] synthesis completed ...");
...
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] xfyun_synth_channel_get");
...
static void xfyun_synth_on_start(apt_task_t *task)
{
	...
	apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] Speak task started");
...
{
	xfyun_synth_channel_t *synth_channel = xfyun_synth_channel_get(task);
	apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] Speak task terminated");
...
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] synth_channel STOP");
	xfyun_synth_channel_t *synth_channel = channel->method_obj;
...
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] synth_channel PAUSE");
	xfyun_synth_channel_t *synth_channel = channel->method_obj;
...
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] synth_channel RESUME");
	xfyun_synth_channel_t *synth_channel = channel->method_obj;
	synth_channel->paused = FALSE;
	/* send asynchronous response */
	mrcp_engine_channel_message_send(channel,response);
...
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] xfyun_synth_channel_set_params ");
...
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] xfyun_synth_channel_get_params ");
	mrcp_synth_header_t *req_synth_header;
	/* get synthesizer header */
	req_synth_header = mrcp_resource_header_get(request);
	if(req_synth_header) {
...
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] xfyun_synth_channel_request_dispatch s%",request->start_line.method_id);
...
static apt_bool_t xfyun_synth_stream_destroy(mpf_audio_stream_t *stream)
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] xfyun_synth_stream_destroy");
	return TRUE;
}
...
static apt_bool_t xfyun_synth_stream_open(mpf_audio_stream_t *stream, mpf_codec_t *codec)
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] xfyun_synth_stream_open");
	return TRUE;
}
...
static apt_bool_t xfyun_synth_stream_close(mpf_audio_stream_t *stream)
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] xfyun_synth_stream_close");
	return TRUE;
}
...
static apt_bool_t xfyun_synth_speak_complete_raise(xfyun_synth_channel_t *synth_channel)
{
	...
	apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] xfyun_synth_speak_complete_raise");
...
static apt_bool_t xfyun_synth_stream_read(mpf_audio_stream_t *stream, mpf_frame_t *frame)
{
	...
	if(synth_channel->stop_response) {
         apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] synth_channel stop_response");
...
static apt_bool_t xfyun_synth_msg_signal(xfyun_synth_msg_type_e type, mrcp_engine_channel_t *channel, mrcp_message_t *request)
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] xfyun_synth_msg_signal");
...
static apt_bool_t xfyun_synth_msg_process(apt_task_t *task, apt_task_msg_t *msg)
{
    apt_log(APT_LOG_MARK, APT_PRIO_INFO, "[xfyun tts] xfyun_synth_msg_process");
